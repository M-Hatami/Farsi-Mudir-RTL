  ...
  var factory = function ($, DataTable) {
    ...
    FixedColumns.prototype = /** @lends FixedColumns.prototype */ {
          ...
          .on("scroll.DTFC", function () {
            if (mouseController === "main") {
              if (that.s.iLeftColumns > 0) {
                that.dom.grid.left.liner.scrollTop =
                  that.dom.scroller.scrollTop;
              }
              if (that.s.iRightColumns > 0) {
                that.dom.grid.right.liner.scrollTop =
                  that.dom.scroller.scrollTop;
              }
            }
          });
        ...
        jqTable
          ...
          .on("destroy.dt.DTFC", function () {
            ...
            $(that.dom.grid.left.liner).off(
              "scroll.DTFC mouseover.DTFC " + wheelType
            );
      ...
      _fnColCalc: function () {
        ...
        $.each(this.s.dt.aoColumns, function (i, col) {
          ...
          if (!th.filter(":visible").length) {
            ...
            if (
              that.s.aiOuterWidths.length ===
              that.s.dt.aoColumns.length - 1
            ) {
      ...
      _fnGridLayout: function () {
        ...
        var iFullHeight = $(
          this.s.dt.nTable.parentNode.parentNode
        ).outerHeight();
        var oOverflow = this._fnDTOverflow();
        ...
        if (this.s.iLeftColumns > 0) {
          ...
          if (oGrid.left.foot) {
            oGrid.left.foot.style.top =
              (oOverflow.x ? oOverflow.bar : 0) + "px"; // shift footer for scrollbar
          }
        ...
        if (this.s.iRightColumns > 0) {
          ...
          if (oGrid.right.foot) {
            oGrid.right.foot.style.top =
              (oOverflow.x ? oOverflow.bar : 0) + "px";
          }
      ...
      _fnCloneRight: function (bAll) {
        ...
        for (
          i = this.s.iTableColumns - this.s.iRightColumns;
          i < this.s.iTableColumns;
          i++
        ) {
      ...
      _fnClone: function (oClone, oGrid, aiColumns, bAll) {
        ...
        if (bAll) {
          ...
          for (i = 0, iLen = aoCloneLayout.length; i < iLen; i++) {
            for (j = 0, jLen = aoCloneLayout[i].length; j < jLen; j++) {
              aoFixedHeader[i][j].cell.className =
                aoCloneLayout[i][j].cell.className;

              // If jQuery UI theming is used we need to copy those elements as well
              $("span.DataTables_sort_icon", aoFixedHeader[i][j].cell).each(
                function () {
                  this.className = $(
        ...
        oClone.body.style.marginBottom =
          dt.oScroll.iBarWidth * 2 + "px"; /* For IE */
          ...
          $(">tbody>tr", that.dom.body).each(function (z) {
            ...
            var i =
              that.s.dt.oFeatures.bServerSide === false
                ? that.s.dt.aiDisplay[that.s.dt._iDisplayStart + z]
                : z;
            var aTds =
              that.s.dt.aoData[i].anCells || $(this).children("td, th");
        ...
        if (dt.nTFoot !== null) {
          if (bAll) {
            ...
            dt.oApi._fnDetectHeader(
              aoCurrFooter,
              $(">tfoot", oClone.footer)[0]
            );

            for (i = 0, iLen = aoCloneLayout.length; i < iLen; i++) {
              for (j = 0, jLen = aoCloneLayout[i].length; j < jLen; j++) {
                aoCurrFooter[i][j].cell.className =
                  aoCloneLayout[i][j].cell.className;
              }
            }
          }
        ...
        var anUnique = dt.oApi._fnGetUniqueThs(
          dt,
          $(">thead", oClone.header)[0]
        );
      ...
      _fnEqualiseHeights: function (nodeName, original, clone) {
        ...
        for (i = 0, iLen = anClone.length; i < iLen; i++) {
          ...
          iHeight =
            iHeightClone > iHeightOriginal ? iHeightClone : iHeightOriginal;
