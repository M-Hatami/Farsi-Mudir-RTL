  ...
  var factory = function ($, DataTable) {
    ...
    KeyTable = function (oInit) {
      ...
      function _fnEventAddTemplate(sKey) {
        ...
        return function (x, y, z) {
          if (
            ...
            alert(
              "Unhandable event type was added: x" + x + "  y:" + y + "  z:" + z
            );
          }
        };
      }
      ...
      function _fnEventRemoveTemplate(sKey) {
        ...
        return function (x, y, z) {
          if (
            (x === null || typeof arguments[0] == "number") &&
            ...
            alert(
              "Unhandable event type was removed: x" +
                x +
                "  y:" +
                y +
                "  z:" +
      ...
      function _fnEventRemove(sType, x, y, fn) {
        ...
        for (
          var i = 0, iLen = _oaoEvents[sType].length;
          i < iLen - iCorrector;
          i++
        ) {
          if (typeof fn != "undefined") {
            if (
              _oaoEvents[sType][i - iCorrector].x == x &&
      ...
      function _fnSetFocus(nTarget, bAutoScroll) {
        ...
        if (
          ...
          if (nTarget.offsetTop + iHeight > iViewportHeight + iScrollTop) {
            dtScrollBody.scrollTop =
              nTarget.offsetTop + iHeight - iViewportHeight;
      ...
      function _fnCellFromCoords(x, y) {
        if (_oDatatable) {
          if (
            typeof _oDatatable.aoData[_oDatatable.aiDisplay[y]] != "undefined"
          ) {
            return _oDatatable.aoData[
              _oDatatable.aiDisplay[y]
            ].nTr.getElementsByTagName("td")[x];
          } else {
            return null;
          }
        } else {
