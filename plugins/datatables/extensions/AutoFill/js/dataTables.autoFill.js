  ...
  var factory = function ($, DataTable) {
    ...
    AutoFill.prototype = {
      ...
      _fnUpdateBorder: function (nStart, nEnd) {
        var border = this.s.border.width,
        ...
        if (this.s.dt.oScroll.sY !== "") {
          ...
          var offsetScroll = $(this.s.dt.nTable.parentNode).offset(),
      ...
      _fnFillerDragStart: function (e) {
        ...
        if (this.s.dt.oScroll.sY !== "") {
          ...
          this.s.scroller.bottom =
            this.s.scroller.top + $(this.s.dt.nTable.parentNode).height();
        }
      ...
      _fnFillerDragMove: function (e) {
        if (
          e.target &&
          e.target.nodeName.toUpperCase() == "TD" &&
          ...
          if (this.c.mode == "y" && coords.x != this.s.drag.startX) {
            e.target = $(
              "tbody>tr:eq(" + coords.y + ")>td:eq(" + this.s.drag.startX + ")",
              this.dom.table
            )[0];
          }
          if (this.c.mode == "x" && coords.y != this.s.drag.startY) {
            e.target = $(
              "tbody>tr:eq(" + this.s.drag.startY + ")>td:eq(" + coords.x + ")",
              this.dom.table
            )[0];
          }
      ...
      _fnFillerFinish: function (e) {
        ...
        for (i = 0, iLen = cells.length; i < iLen; i++) {
          ...
          var stepValue = column.step.call(
            column,
            cell.node,
            read,
            previous,
      ...
      _fnFillerPosition: function (nTd) {
        ...
        filler.style.top =
          offset.top -
          this.s.filler.height / 2 -
          1 +
          $(nTd).outerHeight() +
          "px";
        filler.style.left =
          offset.left -
          this.s.filler.width / 2 -
          1 +
          $(nTd).outerWidth() +
          "px";
      },
      ...
      column: {
        ...
        step: function (cell, read, last, i, x, y) {
          ...
          if (match) {
            return last.replace(
              re,
              parseInt(match[1], 10) + (x < 0 || y < 0 ? -1 : 1)
            );
          }
